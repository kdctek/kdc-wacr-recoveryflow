...
The scaffold, domain model, WA.cr client, WooCommerce integration, background processing, workflow engine with both dispatch actions, admin screens, REST, privacy tooling and this documentation make up the first release line. The form-based workflow editor, the template-variable picker, the Auto Flow recipe with a hook test button, quiet-hours and frequency-cap settings screens and the diagnostics export are planned for slice 2. The Gravity Forms adapter, the documented custom-source example, WP-CLI and a 100,000-row performance pass are planned for slice 3.
